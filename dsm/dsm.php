 ...
 * @license http://blesta/license/ Blesta License Agreement
...
class Dsm extends Module {
	...
	private function getNodeStatistics($node_id, $module_row) {
		...
		// Load the nodes API
		$api->loadCommand("Dsm_nodes");
		$response = null;

		try {
			$nodes_api = new DsmNodes($api);
	...
	/**
	 * Fetches the nodes available for the Dsm server of the given type
	 *
	...
	/**
	 * Fetches the node groups available for the Dsm server of the given type
	 *
	...
	private function getFieldsFromInput(array $vars, $package) {
		...
		$template = $package->meta->set_template == "admin" ? $package->meta->template : (isset($vars['Dsm_template']) ? $vars['Dsm_template'] : null);

		$fields = array(
			'type' => $package->meta->type,
			'hostname' => isset($vars['Dsm_hostname']) ? $vars['Dsm_hostname'] : null,
